...
- 涉及文件：`ob`
- 验证范围：对 `astar-zzz` 返回 `astar-bmc`，对已有 QB_MACHINE 的 machine 返回 QB_MACHINE 值
...
- 涉及文件：`ob`
- 验证范围：对 astar-zzz 机器能找到全部 4 个文件并输出路径
...
- [ ] Step 4: 验证 astar-zzz machine 的 SoC 检测和命令构建（dry-run）

  ```bash
  ob start-qemu astar-zzz --dry-run 2>&1 | head -20
  ```
...
ob start-qemu astar-zzz --dry-run 2>&1 | grep -E '(ast2700|qemu-system-aarch64|loader)' && echo "PASS: AST2700 template" || echo "FAIL: AST2700 template"
